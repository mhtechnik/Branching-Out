...
def filter_users_by_name(name):
    ...
    filtered_users = [user for user in users if str(user.get("name", "")).strip().lower() == needle]
...
def filter_users_by_email(email):
    """Filtert Nutzer mit exakt passender E-Mail (case-insensitive)."""
    users = load_users()
    needle = str(email).strip().lower()
    filtered_users = [user for user in users if str(user.get("email", "")).strip().lower() == needle]
    for user in filtered_users:
        print(user)
